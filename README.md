...
`baseline.py`/`data.py`/`evaluate.py`/`submit.py` need only numpy. The autonomous
agent (`agent.py`) and the causal-history model (`history_model.py`) additionally
require `torch`.
...
2. Run the autonomous agent (screens proposals against validation, logs every
iteration to `run_logs/`, and writes a submission for the validation-selected
champion):

```bash
python3 agent.py --data_dir ./KuaiRand-Pure/data --max_iters 10
```

`--epochs N` optionally caps every experiment's epoch budget for a quick smoke
run (e.g. `--epochs 2`); leave it unset to use each experiment's own tuned
default. Run `python3 agent.py --help` for the full flag list.

3. Once you're done iterating, score the champion against the held-out test
set **exactly once**:
...
This writes `run_logs/final_result.json` with the official-baseline-relative
delta. Do not run it more than once per candidate model — re-running it after
every tweak and keeping the best result is the test-set peeking the ε/patience
convergence protocol below is meant to prevent.

Outputs
- `run_logs/iterations.jsonl` — per-iteration records with hypothesis, code diff, params, metrics, errors.
- `run_logs/best_validation_checkpoint.json` — the current best model by validation primary.
- `run_logs/final_result.json` — the one-time test-set result from `finalize_and_score.py` (see "Current result" below).
- `run_logs/summary.json` and `run_logs/summary.csv` — generated by `python3 summarize_logs.py`, concise per-iteration summaries.

## Current result (2026-08-31, after fixing the finalize bug AND re-screening — see results_summary.md)

| | test primary | vs. official FM baseline |
|---|---|---|
| Official FM baseline | **0.5946** | — |
| causal-history DeepFM, original (unregularized) | 0.5889 | −0.0057 |
| causal-history DeepFM, regularized (dropout=0.3, weight_decay=1e-4) | 0.5909 | **−0.0037** |

**Neither champion beats the baseline on the one-time test check.** We did the two things we
said we'd do — added a second, unbiased validation slice (`log_random_4_22_to_5_08_pure.csv`,
same date window as official valid, 5-seed screening — see `rescreen_champion.py`) and added
regularization — and both candidates still passed that stricter validation gate identically
to the unregularized original. Regularization closed about a third of the test gap but didn't
flip the sign. Read together, this points away from exposure-bias overfitting (which the new
gate specifically targets and didn't catch) and toward **temporal drift**: both validation
slices share validation's date window (Apr 22–28); test is Apr 29–May 8, a week-plus later,
and nothing in the current validation setup probes that gap.

**Recommendation: submit the official FM baseline.** Full detail, including why we stopped at
two one-time test checks rather than trying a third config, is in `results_summary.md`.

Next step, if you want to keep pursuing this direction: make validation itself sensitive to
temporal drift — e.g. leave an explicit gap between train and valid dates instead of them
being adjacent — rather than re-testing the same exposure-bias hypothesis a third way.
...
| `agent_architecture.py` | The autonomous agent controller (registry, selection, convergence, checkpointing). `agent.py` is a thin CLI wrapper around it — both must ship together. |
| `finalize_and_score.py` | One-time, protocol-compliant test-set scoring for whichever model the agent selected as champion. Run this once, at the end, not per-iteration. |
| `requirements.txt` | Pinned dependencies (`numpy`; `torch` for the agent/history model). |

## Bugs found and fixed (2026-08-31)

Two silent bugs meant the shipped submission never actually reflected the validation-selected
champion, and both were hard to notice without deliberately running finalize end-to-end:

1. **`HistoryDeepFMExperiment.finalize()` fed the wrong row type into feature building.**
   It read test rows via `context.splits.get("test") or _read_test_rows(...)`, but
   `context.splits["test"]` (the plain tuple rows from `data.py:encode`) is always truthy,
   so the `_read_test_rows` fallback — which produces the `Row` objects with `.time_ms`/
   `.author`/etc. that `build_causal_features` actually needs — never ran. This crashed with
   `AttributeError: 'tuple' object has no attribute 'time_ms'` every time.
2. **`BenchmarkController.finalize_submission()` swallowed that crash silently** and fell back
   to the plain FM baseline without any warning, so `run_logs/submission.csv` was always FM's
   predictions even when the logs proudly reported `history_deepfm` as the champion. This is
   why the previously reported test delta (+0.0007) looked so much smaller than the model's
   real validation edge (+0.0031) — the "improved" submission was never actually the improved
   model.
3. **`_read_test_rows()` also pointed at a file that doesn't exist** (`log_standard_4_29_to_5_08_pure.csv`)
   and silently returned `[]` — KuaiRand-Pure ships valid+test combined in
   `log_standard_4_22_to_5_08_pure.csv`, split by date, same as `data.py:load()` does.
   It also parsed `upload_dt` as `YYYYMMDD` when the raw field is `YYYY-MM-DD`.

`finalize_submission()` now prints a clear warning and writes `run_logs/finalize_status.json`
if it ever has to fall back to a non-champion model again, instead of failing silently.
With the bug fixed, the real, one-time test score for the champion is reported above under
"Current result" — and it changes the practical recommendation (see that section).
